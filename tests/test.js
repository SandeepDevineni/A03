...
QUnit.test('Testing bmi function with several sets of inputs', function (assert) {
  assert.equal(App.getheight(2, 3), 6, 'Tested with two relatively small positive numbers');
  assert.equal(App.getweight(0, 0), 0, 'Tested with two negative or 0 integers return 0. ');
  assert.equal(App.getheight(12323242422424, 0), 0, 'Tested with two one or two "too large" integers return 0. ');
  assert.equal(App.getheight(0, 'b'), 0, 'Tested with two missing or invalid arguments return 0 ');
  assert.equal(App.getweight('a', 'b'), 0, 'Tested with two relatively string arguments');
  assert.equal(App.getweight(3.4, 2.8), 9.52, 'Tested with two positive non-integer numbers');
});
